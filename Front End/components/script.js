...
login.addEventListener('submit',(event)=>{
  ...
  fetch("http://127.0.0.1:5040/login",
    {method:'POST',
    headers: {
...
const validateSignUp = (fname, lname, email, pass) => {
  return fname && lname && email && pass;
}

const faram = document.querySelector('#signUp');
faram.addEventListener('submit', (event) => {
  event.preventDefault();
  const fname = faram[0].value;
  const lname = faram[1].value;
  const email = faram[2].value;
  const pass = faram[3].value;

  validateSignUp(fname, lname, email, pass) ?
  fetch("http://127.0.0.1:5040/create",
    {method:'POST',
    headers: {
      'Content-Type': 'application/json',
    },
